...
使用说明
引入组件html
    <hrScrollcustom v-on:increment="incrementTotal" ref="scrollcustompopup"></hrScrollcustom>
    
    @incrementTotal：回调函数
        incrementTotal:function(data){//传递到子组件中的回调方法
            console.log(data);//{"data":String,"name":String,"num":String}，data：数据，name：对应的字段，num：对应参照的顺序，以0开始
        },            
        openpopup(){//打开时间遮罩
        ...
        openpopup() {//调用子组件的方法
          this.$refs.scrollcustompopup.openpopup();//子组件的方法
          this.$refs.scrollcustompopup.refresh(val);//刷新子组件的方法，val刷新数据
          //val数据说明：{ type:"",data:[],name:""}，type:sex/blood/custom,data是给自定义内容，data:数据放到data中,name:回传的字段
        },
      ...
      name:'hrscroll',
        ...
        methods:{
            ...
            entersel(){//确定获取数据
                ...
                this.incrementCounter({"data":this.selected[0],"name":this.items.name,"num":num});
                this.popupVisible=false;
            },
            refresh(val){
                this.items=val;
                switch(this.items.type){
                    ...
                    break;
                    case "custom":
                        this.customslots[0].values=this.items.data
