  ...
  <div>
    ...
    <section>
     <ul
        ...
        ref='reply'>
        <li v-for = 'data in dataItem' :ref="data.authorId">
          <div class="top">
            <div class="userimg">
            ...
            <div class="nameAndTime">
              ...
              <div class="time"> {{data.replytime}} </div>
            </div>
          </div>
          <div class="text" tabindex="0"
             @click='handleClick' 
             @blur="handleBlur" 
             :data-authorId = "data.replypsn"
             :data-voiceId = "data.pk_reply">{{ data.replycontent }}</div>
        </li>
      </ul>
    </section>
    <footer>
      <input type="text" name="" placeholder="管理员可回复" v-model='replyInput'
            maxlength="50"
            @click="showpaste" 
            @input='pasteHide' 
            @blur='pasteHide' 
...
import { ProgressBar, HrHeader } from 'hr-ui'
  ...
  export default {
    ...
    mounted(){
      ...
      this.isManage = (getStorage('userinfo').pk_psndoc == this.$route.query.managepsn)?false : true
    },
    methods : {
      getData(){
        let _this = this,
        ...
        ajax('hrssc/portal/psnVoice/queryVoiceReply','get',"application/json",param,function(data) {
          try{
      ...
      deleteText() {
        ...
        ajax('hrssc/portal/psnVoice/deleteReplyByKey','get',"application/json",{'pk_reply':voiceId},function(data){
          Toast(data.message)
      ...
      sendReply : function(){
        ...
        let _this = this,
          param = {
          replyContent : this.replyInput._trim(),
          psndoc : getStorage('userinfo').pk_psndoc,
          pk_psnvoice : this.$route.query.psnvoicepk
        }

        ajax('hrssc/portal/psnVoice/addReply','POST',"application/json",param,function(data){
          if(data.statusCode == 200) {
            Toast(data.message)
            _this.replyInput =''
            _this.dataItem = []
