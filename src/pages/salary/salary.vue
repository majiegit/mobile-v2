<template>
  <div>
    ...
    <van-row type="flex" justify="center" class="header_div">
      ...
      <van-col :span="14" class="header_two">
        <div class="time_div">
          ...
          <span style="font-size: 20px; float: right; margin-right: 10px;" @click="selectDate('beginDate','开始日期')"><van-icon name="notes-o"/></span>
        </div>
      </van-col>
    </van-row>
    ...
    <van-popup
      ...
      closeable
      position="top"
      :style="{ height: '100%',background: 'rgb(245, 245, 245)'}"
    >
      <div style="width: 96%; height: 92%; margin-top: 12%; padding: 2%; overflow-y: auto;">
        <van-row type="flex">
          <van-col span="24" class="detail_title_class">
            <p>{{salaryDetail.wa_classname}}</p>
            <van-divider/>
