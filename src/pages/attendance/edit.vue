      ...
      <div class="item_body" :style="{'height': currentHeight}">
        <van-form input-align="right" ref="billForm" label-width="120">
          ...
          <van-field
            ...
            readonly
            required
            name="original_sign_status"
            :rules="formRules.original_sign_status"
            :value="billInfo.original_sign_status"
          ...
          />
            ...
            <van-field
              ...
              :value="billInfo.fill_type_name"
              @click="selectData(attendanceTypeList,'请选择补考勤原因','fill_type_name', 'name', 'id', billInfo.fill_type_id)"
              label="补考勤类型"
...
export default {
  ...
  methods: {
    ...
    selectOk(selector, item) {
      ...
      this.$set(this.billInfo, selector.field, item[selector.text])
      if (selector.field === 'fill_type_name') {
        this.billInfo.fill_type_id = item[selector.key]
      } else if (selector.field === '') {

      }
      console.log('ssss',this.billInfo)
    },
    // 保存
    saveBillInfo() {
      let params = {
        fill_date: this.billInfo.fill_date,
        fill_reason: this.billInfo.fill_reason,
        fill_type_id: this.billInfo.fill_type_id,
        original_sign_status: this.billInfo.original_sign_status
      }
      this.$refs.billForm.validate(Object.keys(formRules)).then(() => {
        ...
        console.log(params)
        saveAttendanceBill(params).then(res => {
          Toast.clear()
          this.$router.push({name: 'attendanceDetail', query: {pk_h: res.data.primaryKey}})
          console.log(res)
        })
        // 调用保存接口
        // this.$router.push({name: 'attendanceDetail'})
      })
    }
  }
}
